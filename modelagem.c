...
void restricoes(int hidros_num, int vazao_rio, Hidro *hidros, Central *centrais, int central_num){
    for(int i=0; i<hidros_num; i++){    
        ...
        for(int j=0; j<hidros[i].num_arcos; j++){ 
            printf("e%d_%d <= %d;\n", i+1, hidros[i].arcos[j].indice_c, hidros[i].arcos[j].capacidade);         // eij <= wij -> energia que sai da hidreletrica <= capacidade do arco

            printf("e%d_%d = %d*V%d;\n", i+1, hidros[i].arcos[j].indice_c, hidros[i].eficiencia, i+1);          // sum(eij) = Fi x Vi -> o que sai da hidreletrica

            printf("e%d_%d >= 0;\n", i+1, hidros[i].arcos[j].indice_c);         // eij >= 0 -> energia que sai da hidreletrica deve ser maior ou igual a 0
        }
    }

    for(int i=0; i<central_num; i++){   
        for(int j=0; j<centrais[i].num_arcos; j++){
            printf("e%d_%d <= %d;\n", hidros_num+i+1, centrais[i].arcos[j].indice_c, centrais[i].arcos[j].capacidade);          // eij <= wij -> energia que sai da central <= capacidade do arco

            printf("e%d_%d >= 0;\n", hidros_num+i+1, centrais[i].arcos[j].indice_c);            // eij >= 0 -> energia que sai da central deve ser maior ou igual a 0
        }
    }

    for(int i=0; i<central_num; i++){           // sum(eij) - sum(eji) = Di -> o que entra e sai da central deve ser igual a demanda da central
        for(int j=0; j<hidros_num; j++){            // energia que entra na central pelas hidreletricas
            for(int k=0; k<hidros[j].num_arcos; k++){
                if(hidros[j].arcos[k].indice_c == i+1){
                    printf("+e%d_%d ", j+1, i+1);
                }
            }
        }

        for(int j=0; j<central_num; j++){           // energia que entra na central pelas outras centrais
            if(j == i) continue;
            for(int k=0; k<centrais[j].num_arcos; k++){
                if(centrais[j].arcos[k].indice_c == i+1){
                    printf("+e%d_%d ", hidros_num+j+1, i+1);
                }
            }
        }
